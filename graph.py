...
def load_adjacencylist(file_, undirected=False, chunksize=10000, unchecked=True):
  ...
  logging.debug('Parsed {} edges with {} chunks in {}s'.format(total, idx, t1-t0))

  t0 = time()
  G = convert_func(adjlist)
  t1 = time()

  logging.debug('Converted edges to graph in {}s'.format(t1-t0))

  if undirected:
    t0 = time()
    G = G.make_undirected()
    t1 = time()
    logging.debug('Made graph undirected in {}s'.format(t1-t0))
