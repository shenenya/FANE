...
def getCompactDegreeLists(g, root, maxDegree, calcUntilLayer):
    ...
    t1 = time()
    # print('BFS vertex. Time:s', root, t1 - t0)
...
def getDegreeLists(g, root, calcUntilLayer):
    ...
    t1 = time()
    logging.debug('BFS vertex {}. Time: {}s'.format(root, (t1 - t0)))
...
def preprocess_degreeLists():
    logging.debug("Recovering degreeList from disk...")
    degreeList = restoreVariableFromDisk('degreeList')

    logging.debug("Creating compactDegreeList...")
    ...
    logging.debug("compactDegreeList created!")
...
def get_vertices(v, degree_v, degrees, a_vertices):
    ...
    # logging.debug("Selecionando {} próximos ao vértice {} ...".format(int(a_vertices_selected),v))
    vertices = deque()
    ...
    try:
        ...
        # logging.debug("Vértice {} - próximos selecionados.".format(v))
...
def splitDegreeList(part, c, G, compactDegree):
    if (compactDegree):
        logging.debug("Recovering compactDegreeList from disk...")
        degreeList = restoreVariableFromDisk('compactDegreeList')
    else:
        logging.debug("Recovering degreeList from disk...")
        degreeList = restoreVariableFromDisk('degreeList')

    logging.debug("Recovering degree vector from disk...")
...
def calc_distances(part, compactDegree=False):
    ...
    for v1, nbs in vertices.items():
        ...
        for v2 in nbs:
            ...
            t11 = time()
            logging.debug('fastDTW between vertices ({}, {}). Time: {}s'.format(v1, v2, (t11 - t00)))
    ...
    for v1 in vertices:
        ...
        for v2 in list_vertices[cont]:
            ...
            for layer in range(0, max_layer):
                ...
                dist, path = fastdtw(lists_v1[layer], lists_v2[layer], radius=1, dist=dist_func)
                # t1 = time()
                # logging.debug('D ({} , {}), Tempo fastDTW da camada {} : {}s . Distância: {}'.format(v1,v2,layer,(t1-t0),dist))
                distances[v1, v2][layer] = dist
...
def preprocess_consolides_distances(distances, startLayer=1):
    ...
    logging.debug('Distances consolidated.')
...
def exec_bfs_compact(G, workers, calcUntilLayer):
    ...
    # print('Capturing larger degree...')
    maxDegree = 0
    for v in vertices:
        if (len(G[v]) > maxDegree):
            maxDegree = len(G[v])
    # print('Larger degree captured', maxDegree)
    ...
    # print("Saving degreeList on disk...")
    saveVariableOnDisk(degreeList, 'compactDegreeList')
    t1 = time()
    logging.debug('Execution time - BFS: {}m'.format((t1 - t0) / 60))
...
def exec_bfs(G, workers, calcUntilLayer):
    ...
    logging.debug("Saving degreeList on disk...")
...
def generate_distances_network_part1(workers):
    ...
    for part in range(1, parts + 1):
        ...
        logging.debug('Executing part {}...'.format(part))
        ...
        logging.debug('Part {} executed.'.format(part))
...
def generate_distances_network_part3():
    ...
    logging.debug('Weights created.')

    return


def generate_distances_network_part4():
    logging.debug('Consolidating graphs...')
    graphs_c = {}
    layer = 0
    while (isPickle('graphs-layer-' + str(layer))):
        logging.debug('Executing layer {}...'.format(layer))
    ...
    logging.debug("Saving distancesNets on disk...")
    saveVariableOnDisk(graphs_c, 'distances_nets_graphs')
    logging.debug('Graphs consolidated.')
    return
...
def generate_distances_network_part5():
    ...
    logging.debug("Saving nets_weights_alias_method_j on disk...")
...
def generate_distances_network_part6():
    ...
    logging.debug("Saving nets_weights_alias_method_q on disk...")
...
def generate_distances_network(workers):
    t0 = time()
    logging.debug('Creating distance network...')

    os.system("del " + returnPathStruc2vec() + "/pickles/weights_distances-layer-*.pickle")
    with ProcessPoolExecutor(max_workers=1) as executor:
        job = executor.submit(generate_distances_network_part1, workers)
        job.result()
    t1 = time()
    t = t1 - t0
    logging.debug('- Time - part 1: {}s'.format(t))

    t0 = time()
    os.system("del " + returnPathStruc2vec() + "/pickles/graphs-layer-*.pickle")
    with ProcessPoolExecutor(max_workers=1) as executor:
        job = executor.submit(generate_distances_network_part2, workers)
        job.result()
    t1 = time()
    t = t1 - t0
    logging.debug('- Time - part 2: {}s'.format(t))
    logging.debug('distance network created.')

    logging.debug('Transforming distances into weights...')

    t0 = time()
    os.system("del " + returnPathStruc2vec() + "/pickles/distances_nets_weights-layer-*.pickle")
    os.system("del " + returnPathStruc2vec() + "/pickles/alias_method_j-layer-*.pickle")
    os.system("del " + returnPathStruc2vec() + "/pickles/alias_method_q-layer-*.pickle")
    with ProcessPoolExecutor(max_workers=1) as executor:
        job = executor.submit(generate_distances_network_part3)
        job.result()
    t1 = time()
    t = t1 - t0
    logging.debug('- Time - part 3: {}s'.format(t))
    ...
    t1 = time()
    t = t1 - t0
    logging.debug('- Time - part 4: {}s'.format(t))
    ...
    t1 = time()
    t = t1 - t0
    logging.debug('- Time - part 5: {}s'.format(t))
    ...
    t1 = time()
    t = t1 - t0
    logging.debug('- Time - part 6: {}s'.format(t))
