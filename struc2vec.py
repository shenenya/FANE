...
class Graph():
    def __init__(self, g, is_directed, workers, untilLayer=None):
        ...
        # print(" - Converting graph to dict...")
        self.G = g.gToDict()
        ...
        # print('Graph - Number of vertices: ', self.num_vertices)
        # print('Graph - Number of edges: ', self.num_edges)
    ...
    def create_vectors(self):
        logging.debug("Creating degree vectors...")
        degrees = {}
        degrees_sorted = set()
        G = self.G
        ...
        logging.debug("Degree vectors created.")
        logging.debug("Saving degree vectors...")
        saveVariableOnDisk(degrees, 'degrees_vector')

    def calc_distances_all_vertices(self, compactDegree=False):

        logging.debug("Using compactDegree: {}".format(compactDegree))
        if (self.calcUntilLayer):
            logging.debug("Calculations until layer: {}".format(self.calcUntilLayer))
        ...
        with ProcessPoolExecutor(max_workers=self.workers) as executor:
            ...
            for job in as_completed(futures):
                job.result()
                r = futures[job]
                logging.debug("Part {} Completed.".format(r))

        logging.debug('Distances calculated.')
        t1 = time()
        logging.debug('Time : {}m'.format((t1 - t0) / 60))
    ...
    def calc_distances(self, compactDegree=False):
        ...
        with ProcessPoolExecutor(max_workers=1) as executor:
            ...
            logging.debug("Split degree List...")
            part = 1
            for c in chunks:
                job = executor.submit(splitDegreeList, part, c, G, compactDegree)
                job.result()
                logging.debug("degreeList {} completed.".format(part))
                part += 1
